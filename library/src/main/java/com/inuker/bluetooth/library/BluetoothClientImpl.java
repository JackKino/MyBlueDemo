            ...
            protected void onAsyncResponse(int code, Bundle data) {
                ...
                switch (code) {
                    ...
                    case DEVICE_FOUND:
                        ...
                        if (!ByteUtils.isEmpty(device.scanRecord)) {
                            List<Integer> ackchecks=new ArrayList<>();
                            byte [] acks=ByteUtils.trimLast(device.scanRecord);
                            for(int i=0;i<9;i++){
                                ackchecks.add((int) acks[i+2]);
                            }
...
//        BluetoothLog.v(String.format("waitBluetoothManagerReady %s", mCountDownLatch));
    ...
    public void bind(String mac,String pwd, AdvertiseCallback advertiseCallback) {
        ...
        broadcastData[22]=0x00;
        broadcastData[23]=0x00;
